...
function createTaskbarWindowBtn(win) {
  return `<button class="taskbar-window-btn${win.active ? ' active' : ''}"
    onclick="Desktop.focusWindow(${win.id})">${win.title}</button>`;
}
...
function createWindowHeader(win) {
  return `<div class="window-header" onmousedown="Desktop.startDrag(${win.id}, event)">
    ...
    <div class="window-controls">
      <button class="window-control-btn" title="Minimize" onclick="Desktop.minimizeWindow(${win.id})">—</button>
      <button class="window-control-btn" title="Maximize/Restore" onclick="Desktop.toggleMaximizeWindow(${win.id})">⛶</button>
      <button class="window-control-btn" title="Close" onclick="Desktop.closeWindow(${win.id})">✕</button>
    </div>
  </div>`;
}
